if (Test-Path -Path $PSScriptRoot\..\target\x86_64-pc-windows-gnu\release) {
    ...
    wix build -ext WixToolset.UI.wixext $PSScriptRoot\..\deploy\msi\GUP.wxs -b $PSScriptRoot\..\target\msi\x86_64-pc-windows-gnu\release -arch x64 -o $PSScriptRoot\..\target\msi\GUP-x64.msi
}
else {
...
if (Test-Path -Path $PSScriptRoot\..\target\i686-pc-windows-gnu\release) {
    ...
    wix build -ext WixToolset.UI.wixext $PSScriptRoot\..\deploy\msi\GUP.wxs -b $PSScriptRoot\..\target\msi\i686-pc-windows-gnu\release -arch x86 -o $PSScriptRoot\..\target\msi\GUP-x86.msi
}
else {
